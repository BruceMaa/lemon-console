...
public class FileServiceImpl extends BaseServiceImpl<FileMapper, FileDO, FileResp, FileResp, FileQuery, FileReq> implements FileService {
    ...
    protected void beforeDelete(List<Long> ids) {
        ...
        for (var fileDO : fileList) {
            StorageDO storage = storageService.getById(fileDO.getStorageId());
            if (FileTypeEnum.DIR != fileDO.getType()) {
                FileInfo fileInfo = FileConvert.INSTANCE.toFileInfo(fileDO, storage);
                fileStorageService.delete(fileInfo);
            } else {
                // 不允许删除非空文件夹
                boolean exists = baseMapper.lambdaQuery()
                        .eq(FileDO::getParentPath, fileDO.getPath())
                        .eq(FileDO::getStorageId, fileDO.getStorageId())
                        .exists();
                CheckUtils.throwIf(exists, "文件夹 【{}】 不为空，请先删除文件夹下的内容", fileDO.getName());
            }
        }
    }
    ...
    public Long calcDirSize(Long id) {
        ...
        return children.stream().mapToLong(child -> {
            if (FileTypeEnum.DIR == child.getType()) {
                // 递归计算子文件夹大小
    ...
    private FileInfo upload(Object file, String parentPath, String storageCode, String extName) {
        ...
        CheckUtils.throwIf(DisEnableStatusEnum.DISABLE == storage.getStatus(), "请先启用存储 【{}】", storage.getCode());
